...
class DatasetBuilder:
    ...
    def cleanup(self) -> None:
        ...
        def clean(dir_path):
            ...
            # Sử dụng try-except để xử lý trường hợp os.walk trả về danh sách trống
            try:
                [(_, _, files)] = [x for x in os.walk(dir_path)]
            except ValueError:
                # Xử lý trường hợp os.walk trả về danh sách trống
                files = []
            ...
            for f in files:
                ...
                try:
                    os.remove(file_path.replace('\\', '/'))  # Thay thế ký tự \ bằng /
